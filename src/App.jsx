...
import { asSelectableItems } from './modules/capturePile';
import { calculateEndGameBonuses } from './modules/scoring';
...
import { validatePair } from './modules/pairLogic.js';
...
function App() {
  ...
  const canCapture = (card, currentTableItems) => {
    ...
    try {
      const augmented = [...(currentTableItems || []), ...asSelectableItems(player1Pile, player2Pile, currentPlayer)];
      const opts = CaptureValidator.getValidCaptures(card, augmented);
      return Array.isArray(opts) && opts.length > 0;
    } catch (e) {
      console.error('canCapture validation error', e);
      return false;
    }
  };
  ...
  const canBuild = (card, currentTableItems, playerHand) => {
    ...
    // validateBuild signature: (playedCard, selectedItems, playerHand, tableItems, currentPlayer)
    try {
      const augmented = [...(tableItems || []), ...asSelectableItems(player1Pile, player2Pile, currentPlayer)];
      const validation = validateBuild(card, currentTableItems, playerHand, augmented, currentPlayer);
      return validation && validation.isValid;
    } catch (e) {
      console.error('canBuild validation error', e);
      return false;
    }
  };
  ...
  const canPair = (card, currentTableItems, playerHand) => {
    ...
    try {
      return validatePair(card, currentTableItems, playerHand).isValid;
    } catch (e) {
      console.error('canPair validation error', e);
      return false;
    }
  };

  // Suggest a hand card to play that would make a valid build given the current table selection.
  const computeBuildSuggestion = () => {
    // Only suggest when it's this player's turn and they haven't played yet
    if (hasPlayedCard) return null;
    const hand = currentPlayer === 1 ? player1Hand : player2Hand;
    if (!hand || hand.length === 0) return null;
    if (!selectedTableItems || selectedTableItems.length === 0) return null; // need a selection to base suggestion on

    const augmented = [...(tableItems || []), ...asSelectableItems(player1Pile, player2Pile, currentPlayer)];
    for (const candidate of hand) {
      // Try using this candidate as the played card against the current selectedTableItems
      try {
        const validation = validateBuild(candidate, selectedTableItems, hand, augmented, currentPlayer);
        if (validation && validation.isValid) {
          return { card: candidate, message: validation.message, buildValue: validation.buildValue };
        }
      } catch (e) {
        // ignore errors from validateBuild here
      }
    }
    return null;
  };

  const buildSuggestion = computeBuildSuggestion();
  ...
  const playCapture = () => {
    if (selectedCard && selectedTableItems.length > 0 && !hasPlayedCard) {
      ...
      if (captureResult.capturedCards && captureResult.capturedCards.length > 0) {
          if (currentPlayer === 1) {
            // handleCapture already returns updated piles if pile-tops were involved
            if (captureResult.newP1Pile) setPlayer1Pile(captureResult.newP1Pile); else setPlayer1Pile([...player1Pile, ...captureResult.capturedCards]);
            // Hand update moved above state updates
          } else {
            if (captureResult.newP2Pile) setPlayer2Pile(captureResult.newP2Pile); else setPlayer2Pile([...player2Pile, ...captureResult.capturedCards]);
            // Hand update moved above state updates
          }
      }
  ...
  const playBuild = () => {
    if (selectedCard && selectedTableItems.length > 0 && !hasPlayedCard) {
      ...
      if (buildResult.newP1Pile) setPlayer1Pile(buildResult.newP1Pile);
      if (buildResult.newP2Pile) setPlayer2Pile(buildResult.newP2Pile);
  ...
  // Preserve any points already awarded during play (immediate per-card scoring)
  // Now compute only end-of-game bonuses (most cards, most spades) to avoid double-counting
  const { p1Bonus, p2Bonus } = calculateEndGameBonuses(finalP1Pile, finalP2Pile);

    let p1Score = player1Score + p1Bonus;
    let p2Score = player2Score + p2Bonus;
  ...
  const renderTableItem = (item, index) => {
    ...
    if (item.type === 'card') {
      ...
      const suitClass = card.suit === 'H' ? 'hearts' : card.suit === 'D' ? 'diamonds' : card.suit === 'S' ? 'spades' : 'clubs';
      ...
      return (
        <div
          ...
          className={`card table-card ${suitClass} ${isTableItemSelected ? 'selected-table-card' : ''} ${canClickTableItem ? '' : 'disabled'}`}
          onClick={() => canClickTableItem && selectTableItem({ ...card, id: uniqueId })} // Pass unique ID
        >
          <div className="corner-rank">{card.rank}</div>
          <div className="corner-suit">{ { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '\u2660' }[card.suit] }</div>
          <div className="corner-rank br">{card.rank}</div>
          <div className="corner-suit br">{ { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '\u2660' }[card.suit] }</div>
        </div>
      );
    }
  ...
  const renderHandCard = (card, index) => {
     ...
     return (
       ...
       >
          <div className="rank corner-rank" style={{ color: suitColor }}>{card.rank}</div>
          <div className="suit corner-suit" style={{ color: suitColor }}>
          { { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '\u2660' }[card.suit] }
        </div>
        <div className="rank corner-rank br" style={{ color: suitColor }}>{card.rank}</div>
        <div className="suit corner-suit br" style={{ color: suitColor }}>
          { { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '\u2660' }[card.suit] }
        </div>
       </div>
     );
  ...
  // Render the top of a player's captured pile as a selectable, face-up card (single top card shown)
  const renderPileTop = (playerNum) => {
    const pile = playerNum === 1 ? player1Pile : player2Pile;
    const count = Array.isArray(pile) ? pile.length : 0;
    if (!pile || pile.length === 0) {
      return <div className="pile-empty">Pile: 0</div>;
    }

    const topCard = pile[pile.length - 1];
    const virtualId = `pile-${playerNum}-top-${topCard.suitRank}`;
    const suitColor = topCard.suit === 'H' || topCard.suit === 'D' ? 'red' : 'black';
    const isSelected = selectedTableItems.some(i => i.id === virtualId);
    const canClickTop = selectedCard && !hasPlayedCard;

    return (
      <div className="captured-pile" style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
        <div className="pile-badge">{count}</div>
        <div className="pile-label" style={{ fontSize: '0.9em', color: '#ddd', marginBottom: '6px' }}>Pile</div>
        <div
          key={virtualId}
          className={`card pile-card ${isSelected ? 'selected-table-card' : ''} ${canClickTop ? '' : 'disabled'}`}
          style={{ width: 68, height: 104, cursor: canClickTop ? 'pointer' : 'default' }}
          onClick={() => canClickTop && selectTableItem({ type: 'card', id: virtualId, suit: topCard.suit, rank: topCard.rank, suitRank: topCard.suitRank, isPileTop: true, owner: playerNum, source: 'pile' })}
        >
  <div className="rank corner-rank" style={{ color: suitColor }}>{topCard.rank}</div>
  <div className="suit corner-suit" style={{ color: suitColor }}>{ { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '♠' }[topCard.suit] }</div>
  <div className="rank corner-rank br" style={{ color: suitColor }}>{topCard.rank}</div>
  <div className="suit corner-suit br" style={{ color: suitColor }}>{ { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '♠' }[topCard.suit] }</div>
        </div>
      </div>
    );
  };
    ...
    <div className="App">
        ...
        <div className="game-board">
          ...
          <div className={`player-area ${currentPlayer === 1 ? 'current-player' : ''}`}>
            ...
            <div className="player-info">
              ...
              <div className="hand-pile-row">
                <div className="hand"> {player1Hand.map(renderHandCard)} </div>
                <div className="captured-container">{renderPileTop(1)}</div>
              </div>
            </div>
          </div>
            ...
            <h2>Player 2</h2>
             <div className="player-info">
              ...
              <div className="hand-pile-row">
                ...
                <div className="captured-container">{renderPileTop(2)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="actions">
            ...
            {/* Build suggestion: show which hand card would make a valid build with current selection */}
            {buildSuggestion && (
              <p className="hint">Suggestion: Play {buildSuggestion.card.rank}{ { 'C': '♣', 'D': '♦', 'H': '♥', 'S': '♠' }[buildSuggestion.card.suit] || buildSuggestion.card.suit } — {buildSuggestion.message}</p>
            )}
      ...
      {gamePhase === 'gameOver' && (
        <div>
          ...
          <h3>{player1Score > player2Score ? 'Player 1 Wins!' : player2Score > player1Score ? 'Player 2 Wins!' : "It's a Tie!"}</h3>
